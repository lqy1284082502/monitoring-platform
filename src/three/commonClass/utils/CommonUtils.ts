...
export default class CommonUtils {
     ...
     * 墨卡托坐标转换为球面坐标
     * */
    public static lon2xyz(R: number, longitude: number, latitude: number) {
     ...
     * 创建波动光圈
     * */
    public static createWaveMesh = (options: any) => {
        ...
        // 经纬度转球面坐标
        const coord = this.lon2xyz(options.radius * 1.001, options.lon, options.lat);
     ...
     * 创建柱状
     * */
    public static createLightPillar(options: any) {
        ...
        // 经纬度转球面坐标
        const SphereCoord = this.lon2xyz(options.radius, options.lon, options.lat); //SphereCoord球面坐标
     ...
     * 光柱底座矩形平面
     * */
    public static createPointMesh(options: any) {
        const geometry = new PlaneGeometry(1, 1); //默认在XOY平面上
     ...
     * 获取点
     * */
    public static getCirclePoints(option: any) {
        const list = [];
     ...
     * 创建动态的线
     */
    public static createAnimateLine(option: any) {
        // 由多个点数组构成的曲线 通常用于道路
        const l: Vector3[] | undefined = [];
        option.pointList.forEach((e: (number | undefined)[]) => l.push(new Vector3(e[0], e[1], e[2])));
