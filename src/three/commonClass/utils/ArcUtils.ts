...
import CommonUtils from '@/three/commonClass/utils/CommonUtils.ts';
...
export default class ArcUtils {
     ...
     */
    public static flyArc(radius: number, lon1: number, lat1: number, lon2: number, lat2: number, options: any) {
        const sphereCoord1 = CommonUtils.lon2xyz(radius, lon1, lat1); //经纬度坐标转球面坐标
        // startSphereCoord：轨迹线起点球面坐标
        const startSphereCoord = new Vector3(sphereCoord1.x, sphereCoord1.y, sphereCoord1.z);
        const sphereCoord2 = CommonUtils.lon2xyz(radius, lon2, lat2);
        ...
        const startEndQua = this._3Dto2D(startSphereCoord, endSphereCoord);
        // 调用arcXOY函数绘制一条圆弧飞线轨迹
        const arcline = this.arcXOY(radius, startEndQua.startPoint, startEndQua.endPoint, options);
     ...
     * */
    public static _3Dto2D(startSphere: any, endSphere: any) {
     ...
     * 同时在圆弧轨迹的基础上绘制一段飞线
     * */
    public static arcXOY(radius: number, startPoint: Vector3, endPoint: Vector3, options: any) {
        // 计算两点的中点
        ...
        const earthRadianAngle = this.radianAOB(startPoint, endPoint, new Vector3(0, 0, 0));
        /*设置飞线轨迹圆弧的中间点坐标
      ...
      起点、结束点相聚越远，构成的弧线顶部距离球面越高*/
        ...
        const flyArcCenter = this.threePointCenter(startPoint, endPoint, arcTopCoord);
        // 飞线圆弧轨迹半径flyArcR
        const flyArcR = Math.abs(flyArcCenter.y - arcTopCoord.y);
        /*坐标原点和飞线起点构成直线和y轴负半轴夹角弧度值
      参数分别是：飞线圆弧起点、y轴负半轴上一点、飞线圆弧圆心*/
        const flyRadianAngle = this.radianAOB(startPoint, new Vector3(0, -1, 0), flyArcCenter);
        const startAngle = -Math.PI / 2 + flyRadianAngle; //飞线圆弧开始角度
        const endAngle = Math.PI - startAngle; //飞线圆弧结束角度
        // 调用圆弧线模型的绘制函数
        const arcline = this.circleLine(flyArcCenter.x, flyArcCenter.y, flyArcR, startAngle, endAngle, options.color);
        ...
        // 绘制一段飞线，圆心做坐标原点
        const flyLine = this.createFlyLine(flyArcR, startAngle, startAngle + flyAngle, options.flyLineColor);
     ...
     * */
    public static radianAOB(A: any, B: any, O: any) {
     ...
     * */
    public static circleLine(x: any, y: any, r: any, startAngle: any, endAngle: any, color: any) {
     ...
     */
    public static threePointCenter(p1: any, p2: any, p3: any) {
     ...
     */
    public static createFlyLine(radius: number, startAngle: number, endAngle: number, color: string) {
